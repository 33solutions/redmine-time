...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    pattern: ru(
      "клиент\\p{L}*\\s+(?:тупит|не понимает|не читает|не думает)|они не умеют|как обычно у них|опять эти|" +
        "бардак у них|у них бардак|их программист\\p{L}*\\s+(?:не|криво)|рук[аи] не оттуда",
    ),
  ...
  // Текст об инциденте живёт дольше самого инцидента и читается людьми, которые при нём не были.
  // Поэтому из него убирается всё, что фиксирует обязанность и её неисполнение, всё, что распределяет
  // вину, и всё, что подсказывает следующему, как повторить. Остаётся хронология, сделанное и проверка.
  {
    id: "legal-duty",
    title: "Юридическая обязанность и надзор",
    severity: "warn",
    strictForClient: true,
    // «Уведомление о регистрации» — функция сайта, а не надзор: слово «уведомление» ловится
    // только рядом с органом или регулятором.
    pattern: new RegExp(
      `(?:\\bGDPR\\b|${L}(?:` +
        `уведом\\p{L}*[^\\n]{0,40}(?:надзорн\\p{L}*|регулятор\\p{L}*|роскомнадзор\\p{L}*|госорган\\p{L}*|` +
        `(?:уполномоченн|компетентн|контролирующ|государственн)\\p{L}*\\s+орган(?:ов|ами|ам|ах|ы|а|у|ом|е)?)|` +
        `(?:надзорн\\p{L}*|регулятор\\p{L}*|роскомнадзор\\p{L}*)[^\\n]{0,40}уведом\\p{L}*|` +
        `регламентн\\p{L}*\\s+срок\\p{L}*|срок\\p{L}*\\s+подачи|` +
        `срок\\p{L}*\\s+ист[её]к\\p{L}*|ист[её]к\\p{L}*\\s+срок\\p{L}*|` +
        `персональн\\p{L}*\\s+данн\\p{L}*[^\\n]{0,40}(?:утечк\\p{L}*|инцидент\\p{L}*|нарушен\\p{L}*|компрометац\\p{L}*)|` +
        `(?:утечк\\p{L}*|инцидент\\p{L}*|нарушен\\p{L}*|компрометац\\p{L}*)[^\\n]{0,40}персональн\\p{L}*\\s+данн\\p{L}*|` +
        `обязан\\p{L}*\\s+(?:уведом\\p{L}*|сообщить|направить|отчитаться)|должны\\s+были` +
        `)${R})`,
      "giu",
    ),
    hint:
      "Обязанности, надзорные органы и регламентные сроки — предмет юриста, а не задачи в трекере. " +
      "В тексте оставьте хронологию с датами, сделанное и способ проверки.",
  },
  {
    id: "fault-attribution",
    title: "Атрибуция вины",
    severity: "warn",
    strictForClient: true,
    pattern: ru(
      "по (?:нашей |их |чьей-то )?вине|вин[аеы] (?:подрядчик|клиент|заказчик|сотрудник|администратор|провайдер)\\p{L}*|" +
        "наша ошибка|ошибка с нашей стороны|мы не заметили|мы упустили|мы (?:сломали|уронили|потеряли)|" +
        "недосмотр\\p{L}*|халатн\\p{L}*",
    ),
    hint:
      "Вину в тексте не распределяют — ни на нас, ни на клиента, ни на прежних подрядчиков. " +
      "Пишите: что произошло, когда обнаружено, что сделано, как проверено.",
  },
  {
    id: "contractor-blame",
    title: "Оценка чужой работы",
    severity: "warn",
    strictForClient: true,
    pattern: ru(
      "некомпетентн\\p{L}*|" +
        "подрядчик\\p{L}*[^\\n]{0,20}(?:винова\\p{L}*|накосяч\\p{L}*|напорта\\p{L}*)|" +
        "(?:прежн|предыдущ|прошл|бывш)\\p{L}*\\s+(?:подрядчик|исполнител|разработчик|интегратор)\\p{L}*" +
        "[^\\n]{0,30}(?:не\\s+\\p{L}+|плохо|некачественн\\p{L}*|криво|косяч\\p{L}*|намудри\\p{L}*)",
    ),
    hint: "Состояние системы описывается как факт: что обнаружено и что с этим сделано. Кто это сделал — за рамками текста.",
  },
  {
    id: "attack-details",
    title: "Подробности атаки",
    severity: "warn",
    strictForClient: true,
    // Латинские сокращения — с обычными \b: иначе «source» поймается как RCE, а «resource» — как XSS.
    pattern: new RegExp(
      `(?:\\bCVE-\\d{4}-\\d{4,7}\\b|\\bCVE-\\d\\b|\\bXSS\\b|\\bRCE\\b|\\bSSRF\\b|\\bLFI\\b|\\bexploit\\b|\\bpayload\\b|\\bwebshell\\b|` +
        `${L}(?:эксплойт\\p{L}*|инъекц\\p{L}*|б[эе]кдор\\p{L}*|веб-шелл\\p{L}*|шелл\\p{L}*\\s+на\\s+сервер\\p{L}*|` +
        `обход\\p{L}*\\s+(?:авторизац\\p{L}*|аутентификац\\p{L}*|проверк\\p{L}*\\s+прав)|` +
        `закладк\\p{L}*[^\\n]{0,40}восстанавлива\\p{L}*|` +
        `форм\\p{L}*\\s+записи\\s+адрес\\p{L}*[^\\n]{0,40}(?:обойти|обход\\p{L}*)|` +
        `(?:обойти|обход\\p{L}*)[^\\n]{0,40}форм\\p{L}*\\s+записи\\s+адрес\\p{L}*` +
        `)${R})`,
      "giu",
    ),
    hint:
      "Путь проникновения, уязвимый компонент и способ закрепления не публикуются: это и юридический риск, " +
      "и готовая подсказка следующему. Достаточно: «закрыта уязвимость в компоненте X, после обновления не воспроизводится».",
  },
  {
    id: "assertion-not-observation",
    title: "Утверждение о мире вместо наблюдения",
    severity: "warn",
    strictForClient: true,
    pattern: ru(
      "не\\s+утекл\\p{L}*|утечк\\p{L}*[^\\n]{0,20}не\\s+был\\p{L}*|данн\\p{L}*\\s+не\\s+пострадал\\p{L}*|" +
        "никто\\s+не\\s+получил\\s+доступ\\p{L}*|ничего\\s+не\\s+украден\\p{L}*",
    ),
    hint:
      "Утверждение о мире обернётся против автора, если завтра всплывёт обратное. Пишите о наблюдении: " +
      "«признаков доступа к данным покупателей не обнаружено», «после обновления проблема не воспроизводится».",
  },
];
