/**
 * Предотправочная проверка текстов, уходящих в Redmine.
 *
 * Два класса находок:
 ...
 * корректность формулировок — делает модель по разделу «Редполитика» в SKILL.md.
 */
...
const PLACEHOLDER =
  /^(?:[*x•]{3,}|<[^>]*>?|\[[^\]]*\]?|\{[^}]*\}?|(?:см|смотри|в|из|через)(?:[.\s].*)?|скрыт\p{L}*|удал[её]н\p{L}*|выда[её]тся\p{L}*|запрош\p{L}*|хранилищ\p{L}*|n\/?a|—|-+)$/iu;
